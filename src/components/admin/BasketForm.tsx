...
import { addBasketAction, type ActionState } from "@/lib/actions/sumula";

const initialState: ActionState = {};

type TeamWithPlayers = { id: string; name: string; players: { id: string; name: string }[] };

export function BasketForm({ matchId, homeTeam, awayTeam }: { matchId: string; homeTeam: TeamWithPlayers; awayTeam: TeamWithPlayers }) {
  const [state, formAction, pending] = useActionState(addBasketAction, initialState);
  ...
  return (
    <form action={formAction} className="flex flex-wrap items-end gap-2">
      ...
      <div className="flex flex-col gap-1">
        ...
        <select name="points" defaultValue="2" className="rounded-md border border-slate-300 px-2 py-1.5 text-sm">
          <option value="1">1 (lance livre)</option>
      ...
      >
        {pending ? "Adicionando..." : "Adicionar cesta"}
      </button>

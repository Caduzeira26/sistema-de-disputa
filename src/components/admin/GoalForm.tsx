...
import { addGoalAction, type ActionState } from "@/lib/actions/sumula";

const initialState: ActionState = {};

type TeamWithPlayers = { id: string; name: string; players: { id: string; name: string }[] };

export function GoalForm({ matchId, homeTeam, awayTeam }: { matchId: string; homeTeam: TeamWithPlayers; awayTeam: TeamWithPlayers }) {
  ...
  return (
    <form action={formAction} className="flex flex-wrap items-end gap-2">
      ...
      >
        {pending ? "Adicionando..." : "Adicionar gol"}
      </button>
